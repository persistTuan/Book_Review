...
@section("content")
<form class="w-70 d-flex justify-content-center container-fluid " method="POST" action="{{route('book.review',$detailBook[0]->BookID )}}" class="mx-5" >
    @csrf
    <div class="row w-100">
                ...
                <div class="col-sm-10">
                    <select name="selectReviewID" id="selectReviewID">
                        @foreach($detailBook as $detailBookReviewID)
                            <option {{$review->ReviewID==$detailBookReviewID->ReviewID?'selected':''}} value="{{$detailBookReviewID->ReviewID}}">{{$detailBookReviewID->ReviewID}}</option>
                        @endforeach
                    </select>
